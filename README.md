# Elm-Firebase-First-Flight App

This is an Elm app based on the [Frontend Masters](https://frontendmasters.com/) course [Firebase + React: Real-time, Serverless Web Apps](https://frontendmasters.com/courses/firebase-react/). I'm trying to adapt the app to Elm as I follow along.

I adapted the canonical Elm counter app to Firebase Realtime Database. Instead of incrementing the counter value in the model, the model caches the value from Firebase Realtime Database. The `Inc` and `Dec` messages send the incremented or decremented value to JavaScript through a `port` and JavaScript sends Firebase updates to Elm through another port.
